...
sequenceDiagram
    ...
    Note over 社群oauth: 使用者登入成功<br>使用者同意messenger授權
...
sequenceDiagram
    ...
    後端->>資料庫:更新行程(UPDATE)
    資料庫-->>後端:回傳更新成功的資訊
    後端->>FB messenger api:給乘客的訊息
    FB messenger api->>使用者:通知乘客加入行程成功
    後端->>FB messenger api:給司機的訊息
    FB messenger api->>使用者:通知司機有乘客加入
```
...
sequenceDiagram
    ...
    資料庫-->>後端:回傳更新成功的資訊
    後端->>FB messenger api:給乘客的訊息
    FB messenger api->>使用者:通知乘客被加入行程
    後端->>FB messenger api:給司機的訊息
    FB messenger api->>使用者:通知司機乘客加入成功
```
